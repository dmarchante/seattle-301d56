...
function displayPics(){
  ...
  // PUT YOUR RESPONSE IN THIS COMMENT: rando is in block scope of the while loop and cannot be accessed outside the loop.
  console.log(viewed);
...
function handleClick(event) {
  ...
  for(let i = 0; i < names.length; i++){
    if(event.target.id === allProducts[i].name) {
      ...
      console.log(`${event.target.id} has ${allProducts[i].votes} ${allProducts[i].views} views`);
    }
  }
...
function showList() {
  for(let i = 0; i < allProducts.length; i++) {
    ...
    liEl.textContent = `${allProducts[i].name} ${allProducts[i].votes} ${allProducts[i].views} views`;
